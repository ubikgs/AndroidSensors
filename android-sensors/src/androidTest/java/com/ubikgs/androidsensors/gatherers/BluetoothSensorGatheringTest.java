...
public class BluetoothSensorGatheringTest {
    @Rule
    ...
    @Inject @Named("bleSensorChecker") SensorChecker bleSensorChecker;
    ...
    public void testWifiMeasurementsGatherer() throws Exception {
        testSensorDelay(new GathererCreator() {
            @Override
            public BLEMeasurementsGatherer createWithDelay(long delay) {
                return new BLEMeasurementsGatherer(
                        new BasicSensorConfig(delay, new MillisecondsToMicroseconds()), bluetoothManager,
                        sensorEnableRequester, permissionChecker, bleSensorChecker,
    ...
    private interface GathererCreator {
        BLEMeasurementsGatherer createWithDelay(long delay);
    }
}
